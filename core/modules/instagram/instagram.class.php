...
class Instagram {
  ...
  public function getTagMedia($name) {
    return $this->_makeCall('tags/' . $name . '/media/recent', false, array('count' => 200));
  }
  ...
  public function pagination($obj) {
    if (true === is_object($obj) && !is_null($obj->pagination)) {
      if (!property_exists($obj, 'pagination') || !property_exists($obj->pagination, 'next_url')) {
        return array();
      }
      $apiCall = explode('?', $obj->pagination->next_url);
      $function = str_replace(self::API_URL, '', $apiCall[0]);
      (strpos($obj->pagination->next_url, 'access_token') !== false) ? $auth = true : $auth = false;
      return $this->_makeCall($function, $auth, array('count' => 200, 'max_tag_id' => $obj->pagination->next_max_tag_id));
    } else {
